...
**Read `docs/BLADELOOP-PRODUCT-VISION.md` first**, especially §6b and §7.
...
600 °C / 2 mm or 550 °C / 16 mm — and the plant behind it looks **exactly the same either way.**
...
## 🔴 Read this before you open Unity — the scene rule changed
...
2. **Push it immediately** — a small, quick PR.
3. **After that, the stage scenes are Ritwika's.** All your remaining work — tuning how much the
   char stream grows, how dim the kiln goes, how coarse the granules look — happens in the `.cs`
...
> tasks.
>
> My job is making the plant visibly respond to `OrderContext.Model` — the four plant settings.
...
> - Guard everything with `if (OrderContext.HasOrder)` — free play must be unchanged.
>
> Work one stage at a time and check the Unity console after each change.

---

## Priority order — do Stage 4 first

### 1. Stage 4 — the output split 🎯 the money shot
...
// GlassPct: 69.0 (high) → 58.6 (mid) → 46.5 (low)
// CharPct :  5.9 (high) → 15.5 (mid) → 26.5 (low)   ← more than four times
```
...
If time allows: tint the fibre particles by `FiberPurityPct` (93.0 → 82.5 → 69.8), so high grade
reads clean off-white and low grade reads grey and dirty.

### 2. Stage 2 — particle size

`S2_OutputPile_0` … `S2_OutputPile_19` already exist, framed by `CAM_S2_04_OutputGranules`.

| `ParticleSizeMm` | Granules |
|---|---|
| 2 mm (high) | small, uniform, dense — coarse sand |
| 8 mm (mid) | visibly chunkier, more irregular, gaps appearing |
| 16 mm (low) | clearly chunky flakes, loose pile |

Scale each by roughly `particleSizeMm / 2f`, vary each ±25 % with a **fixed seed** so it looks like
shredded material rather than 20 identical cubes, hide some at larger sizes so the pile volume stays
believable, and add random rotation — coarse shred is angular.

Particle size carries weight **0.32** in the model, more than temperature, so this is the second
most valuable stage.

### 3. Stage 3 — temperature and retention
...
> `KilnVisualizer.cs` with its tidy `SetHeat()` is **not in Stage 3** — it belonged to the dashboard
...
Anything you already know is wrong in these scenes — **fold it into the single scene pass.** After
that, send Ritwika the fix rather than opening the file.

### 5. Stage 1 — blade count *(only if 1–4 are done)*
...
- **Do not edit shared material assets** — property blocks only. See the warning above; this has
