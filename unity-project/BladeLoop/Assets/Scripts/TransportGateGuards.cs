...
public class TransportGateGuards : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        postSpot      = new Vector3(68.4f, 0f, -3.0f);   // at the boom, in the opening
    ...
    void Update()
    {
        ...
        // ---- the operator walks out to the post as the load approaches ----
        if (guardOperator != null)
        {
            Vector3 target = x > walkOutAtTruckX ? postSpot : gateHouseSpot;
            ...
            if (arrived) Face(guardOperator, new Vector3(x, 0f, 0f));   // watch the truck in
        }

        // ---- the station guard watches it pass ----
        if (guardStation != null) Face(guardStation, new Vector3(x, 0f, 0f));

        // ---- boom lifts once the truck is close, and stays up ----
        float want = x > raiseAtTruckX ? 1f : 0f;
